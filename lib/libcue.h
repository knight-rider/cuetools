...
CUE_EXPORT struct Cd *cue_parse_file(FILE *);
CUE_EXPORT struct Cd *cue_parse_string(const char *);
CUE_EXPORT void cd_delete(struct Cd *cd);

/* Cd functions */
CUE_EXPORT enum DiscMode cd_get_mode(const struct Cd *cd);
CUE_EXPORT const char *cd_get_cdtextfile(const struct Cd *cd);

// return number of tracks in cd
CUE_EXPORT int cd_get_ntrack(const struct Cd *cd);

// Track functions
CUE_EXPORT struct Track *cd_get_track(const struct Cd *cd, int i);
CUE_EXPORT char *track_get_filename(const struct Track *track);
CUE_EXPORT long track_get_start(const struct Track *track);
CUE_EXPORT long track_get_length(const struct Track *track);
CUE_EXPORT enum TrackMode track_get_mode(const struct Track *track);
CUE_EXPORT enum TrackSubMode track_get_sub_mode(const struct Track *track);
CUE_EXPORT int track_is_set_flag(const struct Track *track, enum TrackFlag flag);
CUE_EXPORT long track_get_zero_pre(const struct Track *track);
CUE_EXPORT long track_get_zero_post(const struct Track *track);
CUE_EXPORT char *track_get_isrc(const struct Track *track);
CUE_EXPORT long track_get_index(const struct Track *track, int i);

// Cdtext functions
CUE_EXPORT struct Cdtext *cd_get_cdtext(const struct Cd *cd);
CUE_EXPORT struct Cdtext *track_get_cdtext(const struct Track *track);
CUE_EXPORT char *cdtext_get(enum Pti pti, const struct Cdtext *cdtext);

CUE_EXPORT struct Rem *cd_get_rem(const struct Cd *cd);
CUE_EXPORT struct Rem *track_get_rem(const struct Track *track);

/*
 * return pointer to value for rem comment
 * @param unsigned int: enum of rem comment
 */
CUE_EXPORT const char* rem_get(unsigned int, struct Rem *);
